...
const generateUniqueID = async (id = authServer.createToken(3, nameOpts, "-"), depth = 0) => {
    if(depth == 3) return;
    depth++;

    const found = await MongoHelper.find("users", { projects: { $elemMatch: { id: id } }});
...
app.get("/*", (req, res) => {
    try {
        ...
        const data = i.indexOf(".html") >= 0 ? (() => {
            const html = fs.readFileSync(`${__dirname}/data/kvizdos/${i}`, 'utf8');
            const $ = cheerio.load(html);
            $('head').prepend(INJECTEDJAVASCRIPT);
            return $.html()
        })() : fs.readFileSync(`${__dirname}/data/kvizdos/${i}`, 'utf8');
